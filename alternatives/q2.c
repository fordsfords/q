...
struct q_msg_s {
	void *d;
	int in_use;
	/* I see a slight performance reduction if I include:
	 *	char msg_pad[CACHE_LINE_SIZE - (sizeof(void *) + sizeof(int))];
	 * so I won't.  :-) */
};
...
struct q_s {
	...
	char deq_pad[CACHE_LINE_SIZE - (2*sizeof(unsigned int))];  /* align next var on cache line */
...
qerr_t q_create(q_t **rtn_q, unsigned int q_size)
{
	...
	if (perr != 0 || q == NULL) { free(q);  return QERR_MALLOCERR; }
